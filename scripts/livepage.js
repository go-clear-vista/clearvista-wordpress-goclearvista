...
const out=process.argv[3]||'/tmp/claude-0/vdlive';
(async () => {
  ...
  for (const [n,vp,m] of [['d',{width:1440,height:900},false],['l',{width:1280,height:800},false],['s',{width:1024,height:768},false],['t',{width:820,height:1180},true],['m',{width:390,height:844},true]]) {
    ...
    const p = await c.newPage(); await p.goto('https://www.goclearvista.com/'+(process.argv[2]||'visual-displays')+'/?nocache='+Date.now(), {waitUntil:'load', timeout:90000});
