...
(async () => {
  ...
  for (const [n,vp,m] of [['d',{width:1440,height:900},false],['t',{width:820,height:1180},true],['m',{width:390,height:844},true]]) {
    ...
    await p.evaluate(h=>{const S=[...document.querySelectorAll('#et-boc .et_pb_section')].filter(s=>!s.closest('header,footer,.et-l--header,.et-l--footer'));const d=document.createElement('div'); d.id='cvvd'; d.innerHTML=h; S[0].before(d); S.forEach(s=>s.remove());const f=document.createElement('div'); f.id='cvform'; f.style.cssText='max-width:1080px;margin:0 auto;padding:20px 0'; f.innerHTML='<iframe style="height:500px;width:60%;margin:0 20%;border:none"></iframe>'; d.after(f);}, html);
